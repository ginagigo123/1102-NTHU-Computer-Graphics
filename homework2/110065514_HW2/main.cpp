...
struct model
{
	Vector3 position = Vector3(0, 0, 0.1); // adjust
	Vector3 scale = Vector3(2, 1, 1); // adjust becasue of window size
...
// --- shader attributes
GLuint iLocView; // viewing matrix
GLuint iLocModel; // model matrix
...
struct iLocLight {
	...
	GLuint constant; // Attenuation 常數
	GLuint linear;
	GLuint quadratic;
}iLocLight[3];
...
void setUniformVariables(GLuint p)
{
	...
	iLocModel = glGetUniformLocation(p, "model_matrix");
	...
	iLocLight[0].ambient = glGetUniformLocation(p, "light[0].Ambient");
	iLocLight[0].diffuse = glGetUniformLocation(p, "light[0].Diffuse");
	iLocLight[0].specular = glGetUniformLocation(p, "light[0].Specular");
	...
	iLocLight[0].constant = glGetUniformLocation(p, "light[0].constant");
	iLocLight[0].linear = glGetUniformLocation(p, "light[0].linear");
	iLocLight[0].quadratic = glGetUniformLocation(p, "light[0].quadratic");

	iLocLight[1].position = glGetUniformLocation(p, "light[1].position");
	iLocLight[1].ambient = glGetUniformLocation(p, "light[1].Ambient");
	iLocLight[1].diffuse = glGetUniformLocation(p, "light[1].Diffuse");
	iLocLight[1].specular = glGetUniformLocation(p, "light[1].Specular");
	...
	iLocLight[1].constant = glGetUniformLocation(p, "light[1].constant");
	iLocLight[1].linear = glGetUniformLocation(p, "light[1].linear");
	iLocLight[1].quadratic = glGetUniformLocation(p, "light[1].quadratic");

	iLocLight[2].position = glGetUniformLocation(p, "light[2].position");
	iLocLight[2].ambient = glGetUniformLocation(p, "light[2].Ambient");
	iLocLight[2].diffuse = glGetUniformLocation(p, "light[2].Diffuse");
	iLocLight[2].specular = glGetUniformLocation(p, "light[2].Specular");
	...
	iLocLight[2].constant = glGetUniformLocation(p, "light[2].constant");
	iLocLight[2].linear = glGetUniformLocation(p, "light[2].linear");
	iLocLight[2].quadratic = glGetUniformLocation(p, "light[2].quadratic");
}
...
void RenderScene(void) {	
	...
	model_matrix = T * R * S;
	glUniformMatrix4fv(iLocModel, 1, GL_FALSE, model_matrix.getTranspose());
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// Next model
	case GLFW_KEY_Z:
		cur_idx = (cur_idx + 1) % MODEL_NUM;
		// cout << "next model, idx:" << cur_idx << endl;
		break;
	// Previous model
	case GLFW_KEY_X:
		cur_idx--;
		if (cur_idx < 0)
			cur_idx = MODEL_NUM - 1;
		// cout << "preious model, idx:" << cur_idx << endl;
		break;
	case GLFW_KEY_T:
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case LightEdit:
		...
		if (lightType == 2)
			light[lightType].spotCutoff += diff_range * 10 * yoffset;
		// other mode - diffuse intensity
		else
			light[lightType].diffuse += Vector3(diff_range * yoffset, diff_range * yoffset, diff_range * yoffset);
		break;
	case ShininessEdit:
		shininess += diff_range * yoffset * 50;
		break;
	}
}
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	switch (cur_trans_mode) {
	case GeoTranslation:
		// cout << "diff: "<< x_diff << ", "<< y_diff << endl;
		// cout << "models[cur_idx].position.x: " << models[cur_idx].position.x << ", " << models[cur_idx].position.y << endl;
	...
	case GeoScaling:
		...
		break;
	case LightEdit:
		light[lightType].position.x += x_diff * diff_range;
		light[lightType].position.y -= y_diff * diff_range;
		break;
	}
}
