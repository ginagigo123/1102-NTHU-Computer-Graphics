...
// --- texture attribute
bool mag_mode_is_nearest = true;
bool min_mode_is_nearest = true;
...
void updateLightVector() {
	for (int i = 0; i < 3; i++) {
		glUniform3f(iLocLight[i].ambient, light[i].ambient.x, light[i].ambient.y, light[i].ambient.z);
		glUniform3f(iLocLight[i].diffuse, light[i].diffuse.x, light[i].diffuse.y, light[i].diffuse.z);
		glUniform3f(iLocLight[i].specular, light[i].specular.x, light[i].specular.y, light[i].specular.z);
		glUniform3f(iLocLight[i].position, light[i].position.x, light[i].position.y, light[i].position.z);

		if (i > 0) {
			glUniform1f(iLocLight[i].constant, light[i].constant);
			glUniform1f(iLocLight[i].linear, light[i].linear);
			glUniform1f(iLocLight[i].quadratic, light[i].quadratic);
		}
	}
	...
	//cout << "Light:" << light[0].ambient << endl << light[0].diffuse << endl;
	//cout << light[0].specular << endl << light[0].position << endl;
}
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		glActiveTexture(GL_TEXTURE0); // before binding texture, Activate texture unit first
		glBindTexture(GL_TEXTURE_2D, models[cur_idx].shapes[i].material.diffuseTexture);
		
		// set repeat mode for texture coordinate 
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);

		// magnification mode
		if (mag_mode_is_nearest)
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		else
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

		// minification mode
		if (min_mode_is_nearest)
		...
		// Eye Loading
		float offset_x = (models[cur_idx].cur_eye_offset_idx / 4) * 0.5;
		float offset_y = 1 - (models[cur_idx].cur_eye_offset_idx % 4) * 0.25;
		glUniform2f(iLocOffset, offset_x, offset_y);
		glUniform1i(iLocIsEye, models[cur_idx].shapes[i].material.isEye);


		// Light
		// vertex or per-pixel lighting	
		glUniform1i(is_perpixel, per_vertex_or_per_pixel);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_L:
			...
			cout << "switch between directional/point/spotlight" << endl;
			cout << "Current light type = " << lightType << endl;
			break;
		// switch magnification texture mode
		case GLFW_KEY_G:
			mag_mode_is_nearest = !mag_mode_is_nearest;
			cout << "mag_mode_is_nearest: " << mag_mode_is_nearest << endl;
			break;
		// switch minification texture mode
		case GLFW_KEY_B:
			min_mode_is_nearest = !min_mode_is_nearest;
			cout << "min_mode_is_nearest: " << min_mode_is_nearest << endl;
			break;
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		// set Eye
		if (materials[i].diffuse_texname.find("EyeDh") != std::string::npos)
			material.isEye = 1;
		else
			material.isEye = 0;
...
void initParameter()
{
	...
	// light
	for (int i = 0; i < 3; i++) {
		light[i].diffuse = Vector3(1.0f, 1.0f, 1.0f);
		light[i].ambient = Vector3(0.15f, 0.15f, 0.15f);
		light[i].specular = Vector3(1.0f, 1.0f, 1.0f);
	}

	// Directional light
	light[0].position = Vector3(1.0f, 1.0f, 2.0f);
	light[0].direction = Vector3(0, 0, 0);

	// Position light
	light[1].position = Vector3(0, 2.0f, 1.0f);
	light[1].constant = 0.01;
	light[1].linear = 0.8;
	light[1].quadratic = 0.1;

	// Spot light
	light[2].position = Vector3(0, 0, 2.0f);
	light[2].spotDirection = Vector3(0, 0, -1.0f);
	light[2].spotExponent = 50;
	light[2].spotCutoff = 30.0;
	light[2].constant = 0.05;
	light[2].linear = 0.3;
	light[2].quadratic = 0.6;
}
...
void setUniformVariables()
{
	...
	// Light
	is_perpixel = glGetUniformLocation(program, "is_perpixel");
	...
	iLocLight[0].position = glGetUniformLocation(program, "light[0].position");
	iLocLight[0].ambient = glGetUniformLocation(program, "light[0].Ambient");
	iLocLight[0].diffuse = glGetUniformLocation(program, "light[0].Diffuse");
	iLocLight[0].specular = glGetUniformLocation(program, "light[0].Specular");
	iLocLight[0].spotDirection = glGetUniformLocation(program, "light[0].spotDirection");
	iLocLight[0].spotCutoff = glGetUniformLocation(program, "light[0].spotCutoff");
	iLocLight[0].spotExponent = glGetUniformLocation(program, "light[0].spotExponent");
	iLocLight[0].constant = glGetUniformLocation(program, "light[0].constant");
	iLocLight[0].linear = glGetUniformLocation(program, "light[0].linear");
	iLocLight[0].quadratic = glGetUniformLocation(program, "light[0].quadratic");

	iLocLight[1].position = glGetUniformLocation(program, "light[1].position");
	iLocLight[1].ambient = glGetUniformLocation(program, "light[1].Ambient");
	iLocLight[1].diffuse = glGetUniformLocation(program, "light[1].Diffuse");
	iLocLight[1].specular = glGetUniformLocation(program, "light[1].Specular");
	iLocLight[1].spotDirection = glGetUniformLocation(program, "light[1].spotDirection");
	iLocLight[1].spotCutoff = glGetUniformLocation(program, "light[1].spotCutoff");
	iLocLight[1].spotExponent = glGetUniformLocation(program, "light[1].spotExponent");
	iLocLight[1].constant = glGetUniformLocation(program, "light[1].constant");
	iLocLight[1].linear = glGetUniformLocation(program, "light[1].linear");
	iLocLight[1].quadratic = glGetUniformLocation(program, "light[1].quadratic");

	iLocLight[2].position = glGetUniformLocation(program, "light[2].position");
	iLocLight[2].ambient = glGetUniformLocation(program, "light[2].Ambient");
	iLocLight[2].diffuse = glGetUniformLocation(program, "light[2].Diffuse");
	iLocLight[2].specular = glGetUniformLocation(program, "light[2].Specular");
	iLocLight[2].spotDirection = glGetUniformLocation(program, "light[2].spotDirection");
	iLocLight[2].spotCutoff = glGetUniformLocation(program, "light[2].spotCutoff");
	iLocLight[2].spotExponent = glGetUniformLocation(program, "light[2].spotExponent");
	iLocLight[2].constant = glGetUniformLocation(program, "light[2].constant");
	iLocLight[2].linear = glGetUniformLocation(program, "light[2].linear");
	iLocLight[2].quadratic = glGetUniformLocation(program, "light[2].quadratic");
	...
	iLocOffset = glGetUniformLocation(program, "eyeOffset");
}
